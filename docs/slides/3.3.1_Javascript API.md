...
- `callback` function to be called when some preference change. This callback
  will receive an object with the new values, 
...
Example usage:
    ...
    !javascript
    MashupPlatform.operator.log('error');
    MashupPlatform.operator.log('info', MashupPlatform.log.INFO);
...
Example usage:
    ...
    !javascript
    MashupPlatform.operator.log('error');
    MashupPlatform.operator.log('warning', MashupPlatform.log.WARN);
