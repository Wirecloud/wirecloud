const Wirecloud = {
    ...
    currentTheme: {
        templates: {
            ...
            'wirecloud/workspace/widget': '<s:styledgui xmlns:s="http://wirecloud.conwet.fi.upm.es/StyledElements" xmlns:t="http://wirecloud.conwet.fi.upm.es/Template" xmlns="http://www.w3.org/1999/xhtml"> <div><div class="wc-widget-heading"><t:grip/><t:errorbutton/><t:titlevisibilitybutton/><t:title/><t:minimizebutton/><t:menubutton/><t:closebutton/></div><t:iframe/><t:leftresizehandle/><t:bottomresizehandle/><t:rightresizehandle/></div></s:styledgui>'
        }
    },
    location: {
    ...
    URLs: {
        ...
        MAC_BASE_URL: new StyledElements.Utils.Template("/showcase/media/%(vendor)s/%(name)s/%(version)s/%(file_path)s"),
        MISSING_WIDGET_CODE_ENTRY: "/api/widget/missing_widget",
        OPERATOR_ENTRY: new StyledElements.Utils.Template("/api/operator/%(vendor)s/%(name)s/%(version)s/html"),
