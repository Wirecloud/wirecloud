...
(function (se, utils) {
    ...
    const ns = Wirecloud.ui.WirecloudCatalogue;

    /*************************************************************************
     *                            Private methods                            *
     *************************************************************************/

    const onUploadFailure = function onUploadFailure(file_entry, msg) {
        this.failures.push({'file': file_entry.name, 'msg': msg});
    };

    const upload_wgt_files = function upload_wgt_files() {
    ...
    const acceptListener = function acceptListener() {
        upload_wgt_files.call(this);
        this.hide();
    };

    const updateEmptyStatus = function updateEmptyStatus() {
    ...
    ns.UploadWindowMenu = class UploadWindowMenu extends Wirecloud.ui.WindowMenu {

        constructor(options) {
            super(utils.gettext("Upload mashable application components"), 'wc-upload-mac-modal');
