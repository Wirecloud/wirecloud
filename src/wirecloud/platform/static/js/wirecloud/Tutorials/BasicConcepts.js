...
(function (utils) {
    ...
    const BS = Wirecloud.ui.Tutorial.Utils.basic_selectors;
    const BA = Wirecloud.ui.Tutorial.Utils.basic_actions;

    const widget_title = function (index) {
        const widget = Wirecloud.activeWorkspace.view.widgets[index];
        return widget.wrapperElement.getElementsByClassName("wc-widget-heading")[0].getElementsByTagName('span')[0];
    };

    const get_menubar = function get_menubar() {
        return document.querySelector(".wiring-sidebar .we-panel-components");
    };

    const widget_menu = function widget_menu(index) {
        const widget = Wirecloud.activeWorkspace.view.widgets[index];
        return widget.wrapperElement.getElementsByClassName('wc-menu-button')[0];
    };

    const enter_keypress = function (e) {
        return e.keyCode === 13;
    };

    const windowForm = function (callback) {
    ...
    const isNotEmpty = function isNotEmpty(input) {
        return input.value !== '';
    };
    ...
    Wirecloud.TutorialCatalogue.add('basic-concepts', new Wirecloud.ui.Tutorial(utils.gettext('Basic concepts'), [
        ...
        {'type': 'autoAction', 'action': BA.click(100), 'elem': BS.button('#wc-user-menu .se-btn')},
