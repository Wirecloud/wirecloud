...
class Migration(DataMigration):
    ...
    def forwards(self, orm):
        ...
        # Migrate data from the old VariableValue model to the Variable model
        for workspace in orm.Workspace.objects.all():
            variable_values = orm.VariableValue.objects.select_related('variable').filter(user=workspace.creator, variable__iwidget__tab__workspace=workspace)
            for variable_value in variable_values:
                variable_value.variable.value = variable_value.value
                variable_value.variable.save()

    def backwards(self, orm):

        # Migrate data from the Variable model to the VariableValue model
        for workspace in orm.Workspace.objects.all():
            variables = orm.Variable.objects.filter(iwidget__tab__workspace=workspace)
            for variable in variables:
                variable.variablevalue_set.create(user=workspace.creator, value=variable.value)
