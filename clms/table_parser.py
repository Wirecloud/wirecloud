...
class TableParser(object):
    ...
    def parse_td(self, td):
        if td.childNodes:
            for td_child in td.childNodes:
                ...
                elif (td_child.tagName == None
                      and not td.getElementsByTagName('table')
                      and not td.getElementsByTagName('thead')
                      and not td.getElementsByTagName('tfoot')
                      and not td.getElementsByTagName('tr')):
    ...
    def _get_num_cols(self, tags):
        ...
        for tag in tags:
            ...
            if not self._get_in_style(tag, 'height') or self._get_in_style(tag, 'height').find("%") != -1:
                print 'x'
                num_cols += (self._get_in_style(tag, 'colspan')
                             and self._get_in_style(tag, 'colspan').isdigit()
    ...
    def _set_width_height_in_cell(self, tags, tr, calculate_width, porcent):
        for tag in tags:
            if calculate_width:
                colspan = (self._get_in_style(tag, 'colspan')
                           and self._get_in_style(tag, 'colspan').isdigit()
                           and int(self._get_in_style(tag, 'colspan')) > 0
                           and int(self._get_in_style(tag, 'colspan'))
                           or 1)
